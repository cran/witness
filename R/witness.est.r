...
function(parameters, guilty=NULL, perp.removed=NULL, data = NULL, ilen = 100, lsize=6, meth="WITC", suspectMatch=FALSE, seedit=TRUE, sample.size=1000, optim = FALSE, goodFoil=FALSE){
		...
		if (is.null(guilty)){
			...
			if (!optim){
				cat("Note: You failed to specify a guilty vector. We will assume that every even row is 'Target Absent' and every odd row is 'Target Present'.\n")
			}
		}
		...
		chi = sum((data-responses)^2/responses)
		sse= sum((data-responses)^2)
		wsse = sqrt(sse/length(responses))
